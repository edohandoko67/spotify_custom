...
class HttpClient {
  ...
  HttpClient(
      {required this.baseUrl, this.enableCache = false, this.showAlert = true, this.duration = const Duration(
          seconds: 15)});
    ...
    ..interceptors
      ...
      InterceptorsWrapper(
          onRequest: (options, handler) {
            ...
            final DateTime? exp = _storage.getExp();
            print('accessToken : $token');
            print('expired_in : $exp');
            if (token != null) {
  ...
  Future<Response<T>> get<T>(String path, {
    ...
    options = Options(
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded', // Pastikan Content-Type yang benar
        }
    );
